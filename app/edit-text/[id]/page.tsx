...
export default function EditText({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const loadText = async () => {
      try {
        const texts = await getAllTexts();
        const foundText = texts.find(t => t.textId === parseInt(params.id));
        if (foundText) {
          setText(foundText);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await updateText(parseInt(params.id), { ...newText, text: processedText });
    ...
    >
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-lg shadow-lg p-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-8">Editar Texto #{params.id}</h1>
